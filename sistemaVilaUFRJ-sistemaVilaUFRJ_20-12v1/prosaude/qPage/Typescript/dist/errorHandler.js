...
export function elementWithObjectError(context, object, element, elementName, line) {
    ...
    Erro ${context ?? "Undefined Context"}. Elemento: ${JSON.stringify(object)}; instância: ${object?.constructor.name ?? "undefined"}
...
export function elementsNotFoundFunction(line, funcName, ...elements) {
  ...
  Erro validando instância obtida para ${funcName || "Undefined Function Name"}`;
    let mappedNullElements = elements.map((element) => element === null || element === undefined ? "null" : element);
    mappedNullElements.forEach((element) => {
        if (element instanceof HTMLInputElement ||
            element instanceof HTMLTextAreaElement ||
