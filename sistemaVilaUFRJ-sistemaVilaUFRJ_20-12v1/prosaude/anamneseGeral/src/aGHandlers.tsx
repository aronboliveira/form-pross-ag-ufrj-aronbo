...
export function searchCEP(cepElement: Element) {
  const cepValue = (cepElement as HTMLInputElement)?.value ?? null;
...
function displayCEPLoadBar(
  cepElement: Element
): [number, number, HTMLProgressElement] | void {
  let progMaxInt;
  let progValueInt;
...
export function addAntMedHandler(click: MouseEvent) {
  if (
  ...
  `;
    ...
    for (let iT = 0; iT < textElements.length; iT++) {
      textElements[iT].addEventListener("input", () =>
        GlobalModel.autoCapitalizeInputs(textElements[iT] as textEl)
      );
    }
    for (let iB = 0; iB < dateBtns.length; iB++) {
      dateBtns[iB].addEventListener("click", (activation) =>
        GlobalHandler.useCurrentDate(
          activation,
          dateBtns[iB] as HTMLButtonElement
        )
      );
    }
  } else if (
