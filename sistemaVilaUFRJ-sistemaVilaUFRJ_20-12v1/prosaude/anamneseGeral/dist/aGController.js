...
if (JSONBtn instanceof HTMLButtonElement && allInputs.length > 0) {
    ...
    JSONBtn.addEventListener("click", () => {
        ...
        if (formDescription &&
            ...
            JSONLink = GlobalHandler.createJSONAnchor(JSONBtn, JSONInpsStoreStringified);
            if (JSONLink) {
                ...
                ErrorHandler.elementNotFound(JSONLink ?? null, "JSONLink", slicedError ?? "NULL");
            }
        }
        else {
...
if (areAllGenContChecked && typeof genValue === "string") {
    gen.addEventListener("change", () => {
        genValue = GlobalModel.fluxGen(gen, gen.value ?? null, genBirthRel, genTrans, genFisAlin);
    });
...
if (emailInputs.length > 0) {
    emailInputs.forEach((emailInput) => {
        if (emailInput instanceof HTMLInputElement) {
            emailInput.addEventListener("click", () => AGModel.addEmailExtension(emailInput));
            emailInput.addEventListener("input", () => AGModel.addEmailExtension(emailInput));
        }
        else {
            const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
...
if (antFamChecks.length > 0) {
    antFamChecks.forEach((antFamCheck) => {
        if (antFamCheck instanceof HTMLInputElement) {
            ...
            antFamCheck.addEventListener("dblclick", () => GlobalHandler.doubleClickHandler(antFamCheck));
        }
        else {
            const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
...
if (cepElement &&
    ...
    cepElement.addEventListener("input", () => {
        const isCepBtnOff = AGHandlers.enableCEPBtn(cepElement.value.length, cepElementBtn);
        if (cepElementBtn &&
            cepElementBtn instanceof HTMLButtonElement &&
            !isCepBtnOff) {
            cepElementBtn.addEventListener("click", () => AGHandlers.searchCEP(cepElement));
        }
