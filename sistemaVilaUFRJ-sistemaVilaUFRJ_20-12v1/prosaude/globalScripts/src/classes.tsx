...
import { formClassPerson, formPerson } from "./types";
...
export class Person {
  ...
  checkAtvLvl(person: formClassPerson) {
  ...
  ): [string, number, number] | undefined | never {
    try {
      if (
        person &&
        "weight" in person &&
  ...
  calcPGC(person: formClassPerson) {
  ...
  calcGET(TMB: number, factorAtvLvl: number) {
    if (TMB && factorAtvLvl) {
  ...
  static clonePerson(person: formPerson) {
