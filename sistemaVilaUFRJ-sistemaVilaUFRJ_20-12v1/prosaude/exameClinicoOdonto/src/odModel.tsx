...
export function resetAvDentValue(
  selectInp: HTMLInputElement | HTMLSelectElement
) {
  ...
  if (
    ...
    for (let i = 0; i < dlOptionsArray.length; i++) {
      if ((dlOptionsArray[i] as HTMLOptionElement).value === targValue) {
        isValuePreDef = true;
        break;
      }
    }
      ...
      if (firstLabNumStr !== null) {
        for (let i = 0; i < labsNList.length; i++) {
          ...
          if (labsNList[i] instanceof HTMLElement)
            (labsNList[i] as HTMLElement).style.setProperty("order", nOrder);
        }
      }
    } else {
      const slicedError =
