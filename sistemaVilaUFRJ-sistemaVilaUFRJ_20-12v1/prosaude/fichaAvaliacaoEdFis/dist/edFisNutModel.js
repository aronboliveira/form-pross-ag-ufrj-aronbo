...
import { Person } from "../../globalScripts/dist/classes.js";
                ...
                if (cols.every((col) => col instanceof HTMLTableColElement)) {
                    ...
                    for (let j = 0; j < cols.length; j++) {
                        ...
                        if (childInstance !== `HTMLTableColElement`) {
                            ...
                            ErrorHandler.elementNotFound(cols[j] ?? null, "child <col>", slicedError ?? "NULL");
                        }
                    }
...
export function changeTabDCutLayout(protocolo, tabDC) {
    ...
    if (protocolo &&
            ...
            if (optionElementMatch3) {
                ...
                if (arrayTabIds && arrayTabIds.length !== tabDC.rows.length) {
                    ...
                    if (bodyType.value === "masculino" || bodyType.value === "feminino") {
                        ...
                        const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
                        ErrorHandler.stringError("validando .value de bodyType", bodyType?.value ?? null, slicedError ?? "NULL");
                    }
                }
                else {
                ...
                const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
                ErrorHandler.stringError("obtendo pollock.value", protocolo?.value ?? null, slicedError ?? "NULL");
                return "pollock3";
            }
        }
    }
        ...
        const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
        ErrorHandler.elementNotFound(bodyType ?? null, "bodyType", slicedError ?? "NULL");
        return "pollock3";
    }
    return "pollock3";
}
    ...
    if (tab.id === "tabDCut") {
        ...
        for (let iR = 0; iR < tableRows.length; iR++) {
            ...
            if (rowIdMatch) {
                ...
                const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
                ErrorHandler.stringError(`obtendo id da row ${tableRows[iR] ?? null}`, rowId ?? null, slicedError ?? "NULL");
            }
        }
    }
    else {
        const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
        ErrorHandler.stringError(`obtendo id da table ${tab ?? null}`, tab.id ?? null, slicedError ?? "NULL");
    }
    return arrayTabIds;
}
...
export function isPGCDecaying(person, PGC, targInpPGC) {
    ...
    if (!(spanRoundingAlertIcon instanceof HTMLSpanElement)) {
        const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
        ErrorHandler.elementNotFound(spanRoundingAlertIcon ?? null, "spanRoundingAlertIcon", slicedError ?? "NULL");
    }
    else {
    ...
    if (decreasedPerson) {
        ...
        const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
        ErrorHandler.objectError("decreasedPerson", person ?? null, "person", "6", slicedError);
    }
