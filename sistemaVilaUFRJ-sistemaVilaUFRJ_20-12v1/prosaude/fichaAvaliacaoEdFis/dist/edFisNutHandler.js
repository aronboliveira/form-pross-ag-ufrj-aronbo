    ...
    if (locksTabInd.length > 0 &&
        ...
        if (autoFillActivation) {
            locksTabInd.forEach((lock) => {
                const siblingInput = GlobalHandler.searchPreviousSiblings(lock, "inpInd");
          ...
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-lock" viewBox="0 0 16 16">
            <path d="M8 1a2 2 0 0 1 2 2v4H6V3a2 2 0 0 1 2-2m3 6V3a3 3 0 0 0-6 0v4a2 2 0 0 0-2 2v5a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2M5 8h6a1 1 0 0 1 1 1v5a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V9a1 1 0 0 1 1-1"/>
                    ...
                    const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
                    ErrorHandler.inputNotFound(siblingInput ?? null, "siblingInput", slicedError ?? "NULL");
                }
            });
        }
        else {
...
export function getNumCol(evEl) {
    ...
    if ((evEl && evEl.id.match(/[0-9]+_[0-9]+$/g)) ||
        ...
        const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
        ErrorHandler.matchError(".id do Elemento de Evento", evEl ?? null, evEl?.id ?? "null", slicedError ?? "NULL");
    }
    return numCol;
}
export function validateEvResultNum(evEl, property) {
    if (evEl instanceof HTMLInputElement && evEl.type === "number") {
        ...
        retornada como NaN e revertida para 0.`);
            ...
            const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
            ErrorHandler.typeError(`property relacionada a ${evEl?.id ?? "undefined Event Element"}`, property, "number", slicedError ?? "NULL");
            property = 0;
        }
    }
    else {
        const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
        ErrorHandler.elementNotFound(evEl, `${evEl?.id ?? "undefined Event Element"}`, slicedError ?? "NULL");
        property = 0;
    }
    return property;
}
...
function updateIMCMLGContext(IMCMLGArray, gordCorpLvl, targInpIMC, targInpMLG, formTMBTypeElement, ignoredIndex) {
    ...
    if (gordCorpLvl instanceof HTMLSelectElement &&
        ...
        if (!(ignoredIndex === "MLG" || ignoredIndex === "BOTH") &&
            ...
            const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
            ErrorHandler.typeError("IMCMLGArray[2]", IMCMLGArray[2] ?? null, "number", slicedError ?? "NULL");
        }
        if (!(ignoredIndex === "IMC" || ignoredIndex === "BOTH") &&
            ...
            const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
            ErrorHandler.typeError("IMCMLGArray[1]", IMCMLGArray[1] ?? null, "number", slicedError ?? "NULL");
        }
        if (typeof IMCMLGArray[0] === "string") {
            ...
            const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
            ErrorHandler.typeError("IMCMLGArray[0]", IMCMLGArray[0] ?? null, "string", slicedError ?? "NULL");
        }
        if (IMCMLGArray[0] === "" || IMCMLGArray[1] === 0 || IMCMLGArray[2] === 0) {
            console.warn(`IMCMLGArray não atualizado.
      Valores obtidos: ${IMCMLGArray[0] ?? "null"}; ${IMCMLGArray[1] ?? 0}; ${IMCMLGArray[2] ?? 0} }`);
        ...
        const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
        ErrorHandler.multipleElementsNotFound(slicedError ?? "NULL", "instâncias de elementos em updateIMCMLGContext", gordCorpLvl ?? null, targInpIMC ?? null, targInpMLG ?? null, formTMBTypeElement ?? null);
...
export function fluxFormIMC(IMC, formTMBTypeElement, gordCorpLvl) {
    if (formTMBTypeElement instanceof HTMLSelectElement &&
      ...
      Valor obtido: ${IMC ?? "NaN"}`);
        ...
        const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
        ErrorHandler.multipleElementsNotFound(slicedError ?? "NULL", "obtendo formTMBTypeElement e/ou gordCorpLvl Element", formTMBTypeElement ?? null, gordCorpLvl ?? null);
    }
}
...
export function matchTMBElements(mainSelect, formTMBTypeElement, spanFactorAtleta, gordCorpLvl, lockGordCorpLvl, IMC) {
    ...
    if (mainSelect instanceof HTMLSelectElement &&
      ...
      <path d="M8 1a2 2 0 0 1 2 2v4H6V3a2 2 0 0 1 2-2m3 6V3a3 3 0 0 0-6 0v4a2 2 0 0 0-2 2v5a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2M5 8h6a1 1 0 0 1 1 1v5a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V9a1 1 0 0 1 1-1"/>
        ...
        const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
        ErrorHandler.multipleElementsNotFound(slicedError ?? "NULL", "argumentos em matchTMBElements()", mainSelect ?? null, formTMBTypeElement ?? null, spanFactorAtleta ?? null, gordCorpLvl ?? null, lockGordCorpLvl ?? null);
...
export function updatePGC(person, numRef, context, parentElement) {
    ...
    if (context === "cons" || context === "tab") {
        ...
        const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
        ErrorHandler.stringError("validando contexto.  Valores de string aceitos: cons || tab.", context ?? "undefined", slicedError ?? "NULL");
    }
    if (targInpSumDCut instanceof HTMLInputElement &&
        ...
        const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
        ErrorHandler.inputNotFound(targInpSumDCut, "targInpSumDCut", slicedError ?? "NULL");
    }
...
export function updateAtvLvl(mainSelect, atvLvl, secondarySelect) {
    ...
    if (typeof returnedAtvLvl === "string") {
        ...
        const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
        ErrorHandler.typeError("atualização de mainSelect em updateAtLvl()", returnedAtvLvl ?? null, "string", slicedError ?? "NULL");
    }
    return atvLvl;
}
export function defineTargInps(numRef, context, parentEl) {
    ...
    if (typeof numRef === "string") {
        ...
        if (numRef && numRef !== "") {
            ...
            const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
            ErrorHandler.stringError("convertendo Número de Consulta de string para número", numRef, slicedError ?? "NULL");
        }
    }
    if (typeof numRef === "number") {
        if (context === "cons" || context === "tab") {
            ...
            const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
            ErrorHandler.typeError("numRef", numRef ?? null, "number ou string somente com números", slicedError ?? "NULL");
        }
    }
    else {
        const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
    ...
    if (arrayTargInps.length === 6) {
        for (let iA = 0; iA < arrayTargInps.length; iA++) {
            if (arrayTargInps[iA] instanceof HTMLInputElement &&
                ...
                const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
                ErrorHandler.inputNotFound(arrayTargInps[iA], `arrayTargInps ${enumTargInpTypes[iA]}`, slicedError ?? "NULL");
        ...
        const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
        ErrorHandler.elementNotPopulated(arrayTargInps ?? null, "arrayTargInps", slicedError ?? "NULL");
    }
        ...
        `;
            if (tBodyContainer) {
                ...
                for (let iT = 0; iT < textElements.length; iT++) {
                    textElements[iT].addEventListener("input", () => GlobalModel.autoCapitalizeInputs(textElements[iT]));
                }
                for (let iN = 0; iN < numInps.length; iN++) {
                    numInps[iN].addEventListener("input", () => GlobalModel.numberLimit(numInps[iN]));
                }
            }
            rowCountAtivFisRot++;
        }
    ...
    `;
        if (parentTab) {
            ...
            for (let iB = 0; iB < textElements.length; iB++) {
                textElements[iB].addEventListener("input", () => GlobalModel.autoCapitalizeInputs(textElements[iB]));
            }
            rowCountComorb++;
        }
    }
...
export function switchRequiredCols(elements) {
    if (elements.length > 0 && elements[1] instanceof HTMLSelectElement) {
        ...
        let returnedNum = parseInt(GlobalHandler.updateSimpleProperty(numConsElement) ?? "0", 10);
        ...
        if (typeof returnedNum === "number" &&
            ...
            if (totalRows && totalRows.length > 0) {
                ...
                const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
                ErrorHandler.elementNotPopulated(totalRows ?? null, "NodeList de elementos <tr> em switchRequiredCols()", slicedError ?? "NULL");
            }
            ...
            if (totalCols && totalCols.length > 0) {
                ...
                const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
                ErrorHandler.elementNotPopulated(totalCols ?? null, "NodeList de elementos <col> em switchRequiredCols()", slicedError ?? "NULL");
            }
            ...
            if (inpsCells.length > 0 &&
                ...
                inpsCells.forEach((inpCel) => {
                    if (inpCel instanceof HTMLInputElement) {
                        ...
                        const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
                        ErrorHandler.inputNotFound(inpCel ?? null, "inpCel", slicedError ?? "NULL");
                    }
                });
            }
            else {
        ...
        Array de Validação para NodeLists obtido: ${JSON.stringify(validInpsNodeLists) ?? "undefined"}`);
            ...
            if (flatRequiredCells.length > 0 &&
                ...
                const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
                ErrorHandler.elementNotPopulated(flatRequiredCells ?? null, "flatRequiredCells", slicedError ?? "NULL");
            }
        }
        else {
...
export function switchNumConsTitles(consTitles, trioEl, numTitledCons, numTabs) {
...
export function createArraysRels(btnId, arrayRows, protocolValue) {
        ...
        if (arrayConsultasNum &&
            ...
            switch (protocolValue) {
                ...
                default:
                    ...
                    ErrorHandler.elementNotFound(protocolValue ?? null, "protocolo", slicedError ?? "NULL");
            }
                        ...
                        if (iR < btnRow - 2) {
                            ...
                            if (tbodyQuery) {
                                ...
                                if (tBodyChildren &&
                                    ...
                                    const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) ||
                                        "";
                                    ErrorHandler.elementNotPopulated(tBodyChildren ?? null, "tBodyChildren", slicedError ?? "NULL");
                                }
            ...
            if (numRow !== arrayRows.length) {
                for (let iCol = 0; iCol < arrayConsultasNum.length; iCol++) {
                    ...
                    if (targCelInp && targCelInp instanceof HTMLInputElement) {
                        ...
                        const slicedError = new Error().stack?.split("\n")[1]?.trim()?.slice(-7, -1) || "";
                        ErrorHandler.inputNotFound(targCelInp ?? null, "targCelInp", slicedError ?? "NULL");
                    }
