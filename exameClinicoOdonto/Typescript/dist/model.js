...
export function resetAvDentValue(selectInp) {
    ...
    if (targInp instanceof HTMLInputElement ||
        ...
        if (dlOptionsArray.every((dlOption) => dlOption instanceof HTMLInputElement ||
            dlOption instanceof HTMLTextAreaElement)) {
            for (let i = 0; i < dlOptionsArray.length; i++) {
                const currenOption = dlOptionsArray[i];
            ...
            if (targInp && isValuePreDef && targInp instanceof HTMLElement) {
                const inputElement = targInp;
                setTimeout(() => {
                    inputElement.value = "";
                    isValuePreDef = false;
                }, 100);
                targInp.setAttribute("placeholder", "Apagado");
                let placeholderTimer = setTimeout(() => inputElement.classList.add("placeholder-hidden"), 1000);
