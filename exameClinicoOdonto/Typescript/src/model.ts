  ...
  if (
    ...
    if (
      ...
      for (let i = 0; i < dlOptionsArray.length; i++) {
        const currenOption = dlOptionsArray[i] as
          | HTMLInputElement
          | HTMLTextAreaElement;
        if (currenOption.value === targValue) {
          isValuePreDef = true;
          break;
        }
      }
      if (targInp && isValuePreDef && targInp instanceof HTMLElement) {
        const inputElement = targInp as HTMLInputElement | HTMLTextAreaElement;
        setTimeout(() => {
          inputElement.value = "";
          isValuePreDef = false;
        }, 100);
...
export function orderLabels() {
  subDivsQuadrs.forEach((subDiv) => {
    let labsNList: NodeListOf<HTMLLabelElement> =
      subDiv.querySelectorAll(".labelAvDent");
