...
import { targEl, textEl } from "../../globalScripts/src/types.js";

let movingSrcItem: targEl = null;
let targItem = null;
...
export function addTextToObs(click: MouseEvent, inspLIBtn: HTMLButtonElement) {
...
export function dragStart(move: DragEvent) {
  movingSrcItem = (move?.target as HTMLElement) ?? null;
...
export function dragStartChilds(contInQuadrs: Element[] | NodeListOf<Element>) {
  contInQuadrs.forEach((contInQuadr) => {
    contInQuadr.setAttribute("draggable", "true");
  });
}

export function dragEnter(move: DragEvent) {
  move.preventDefault();
}

export function dragOver(move: DragEvent) {
  move.preventDefault();
}

export function dragLeave(move: DragEvent) {
  move.preventDefault();
}

export function dragDrop(drop: DragEvent) {
...
export function dragEndChilds(contInQuadrs: Element[] | NodeListOf<Element>) {
  contInQuadrs.forEach((contInQuadr) => {
...
export function resetLabels(quadrBtn: HTMLButtonElement) {
...
export function clearQuadrInps(quadrInp: Element) {
    ...
    if (quadrInp.nextElementSibling) {
      ...
      for (let i = 0; i < dlOptions.length; i++) {
        if (dlOptions[i] instanceof HTMLOptionElement) {
          dlOptionsValues.push((dlOptions[i] as HTMLOptionElement).value);
        }
      }
...
export function addSubDivTrat(click: MouseEvent) {
  if (
    click.target &&
    ...
    if (click.target.classList.contains("addTrat")) {
      ...
      for (let iD = 0; iD < dateBtns.length; iD++) {
        dateBtns[iD].addEventListener("click", (activation) =>
          GlobalHandler.useCurrentDate(
            activation,
            dateBtns[iD] as HTMLButtonElement
          )
        );
      }
      for (let iT = 0; iT < textConts.length; iT++) {
        textConts[iT].addEventListener("input", () =>
          GlobalModel.autoCapitalizeInputs(textConts[iT] as textEl)
        );
      }
      for (let iA = 0; iA < astDigtBtns.length; iA++) {
        astDigtBtns[iA].addEventListener("click", (activation: Event) =>
          GlobalHandler.changeToAstDigit(
            activation as MouseEvent,
            astDigtBtns[iA] as HTMLButtonElement
          )
        );
      }
