export function elementNotFound(element, elementName) {
    ...
    console.error(`Erro validando instância de ${element.id || elementName || "null"}.
...
export function elementWithArrayError(context, array, element) {
  ...
  Instância de sumDCBtn obtido: ${Object.prototype.toString.call(element)?.slice(8, -1) ?? "null"}.`);
}
export function elementWithObjectError(context, object, element) {
    console.error(`Erro ${context}. Elemento: ${object}; instância: ${object?.constructor.name ?? "undefined"}
    gordCorpLvl: instância obtida: ${Object.prototype.toString.call(element)?.slice(8, -1) ?? "null"}`);
}
...
export function multipleElementsNotFound(context, ...elements) {
    ...
    let mappedNullElements = elements.map((element) => element === null || element === undefined ? "null" : element);
...
export function elementsNotFoundFunction(funcName, ...elements) {
    let errorMessage = `Erro validando instância obtida para ${funcName || "null"}`;
    let mappedNullElements = elements.map((element) => element === null || element === undefined ? "null" : element);
    mappedNullElements.forEach((element) => {
        errorMessage += `Instância de ${element.id || "null"} obtida: ${Object.prototype.toString.call(element)?.slice(8, -1) ?? "null"};\n`;
    });
...
export function objectError(context, object, maxPropertiesNumber) {
  ...
  Número obtido de propriedades: ${Object.keys(object).length ?? 0}; Número aceito: ${maxPropertiesNumber ?? 0}`);
}
