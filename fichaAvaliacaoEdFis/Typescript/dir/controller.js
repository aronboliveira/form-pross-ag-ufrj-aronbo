...
if (consTablesFs && consTablesFs instanceof HTMLFieldSetElement) {
    ...
    if (allTabledInps.length > 0) {
        ...
        console.error(`Erro validando inputs internos ao <fieldset> de tabelas métricas`);
    }
  ...
  Instância obtida: ${Object.prototype.toString.call(consTablesFs).slice(8, -1) ?? "null"}.`);
}
    ...
    if (tabSVi &&
      ...
      Número máximo obtido: ${parseInt(numConsElement?.lastElementChild?.value ?? "1", 10)}`);
                    ...
                    if (typeof person.weight === "number") {
                        targInpWeight.addEventListener("input", () => {
                            ...
                            if (isAutoFillActive) {
                                ...
                                indexesArray = Handlers.updateIndexesContexts(person, gordCorpLvl, targInpIMC, targInpMLG, targInpTMB, targInpGET, formTMBTypeElement, factorAtvLvl, factorAtleta);
                                IMC = indexesArray[0];
                                MLG = indexesArray[1];
                                TMB = indexesArray[2];
                                GET = indexesArray[3];
        ...
        .type obtido: ${targInpHeight?.type ?? "null"}`);
            ...
            Instância obtida: ${Object.prototype.toString.call(ageElement).slice(8, -1) ?? "null"};
            .type obtido: ${ageElement?.type ?? "null"}.`);
                    }
                  ...
                  Valor obtido: ${textBodytype?.value ?? "null"}`);
                    ...
                    if (targInpSumDCut instanceof HTMLInputElement) {
                        ...
                        targInpSumDCut.addEventListener("input", () => {
                            ...
                            const returnedSumDCut = Handlers.updateSimpleProperty(targInpSumDCut) || 0.01;
                            if (typeof returnedSumDCut === "number") {
                                person.sumDCut = returnedSumDCut;
                            }
                            else if (typeof returnedSumDCut === "string") {
                                person.sumDCut = parseInt(returnedSumDCut.replaceAll(/[^0-9.,+-]/g, "") || "0.01", 10);
                            }
                            PGC = person.calcPGC(person);
                    ...
                    if ((person instanceof Man ||
                        ...
                        if (autoFillBtn instanceof HTMLButtonElement) {
                            autoFillBtn.addEventListener("click", () => {
                                isAutoFillActive = Handlers.switchAutoFill(autoFillBtn, locksTabInd);
                            });
                        }
                        else {
                            console.error(`Erro validando Botão de Cálculo Automático.
              Instância obtida: ${Object.prototype.toString.call(autoFillBtn).slice(8, -1) ??
                                "null"}`);
                        }
                        ...
                        if (atvLvlElement instanceof HTMLSelectElement &&
                            ...
                            Tipo primitivo obtido: ${typeof returnedfactorAtvLvl}`);
                                    ...
                                    Handlers.fluxFormIMC(IMC ?? 0, formTMBTypeElement, gordCorpLvl);
