...
if (ageElement instanceof HTMLInputElement && ageElement.type === "number") {
  ...
  Instância obitda: ${Object.prototype.toString.call(ageElement).slice(8, -1) ?? "null"};
...
if (atvLvlElement instanceof HTMLSelectElement) {
  ...
  Instância obtida: ${Object.prototype.toString.call(atvLvlElement).slice(8, -1) ?? "null"};
...
if (tabDC && tabDC instanceof HTMLTableElement) {
    ...
    if (protocolo && protocolo instanceof HTMLSelectElement) {
        ...
        if (textBodytype && textBodytype instanceof HTMLSelectElement) {
            ...
            console.warn(`Erro validando campo de Bodytype. Elemento: ${protocolo}, instância: ${Object.prototype.toString.call(textBodytype).slice(8, -1) ?? "null"}`);
        }
    }
    else {
        console.warn(`Erro validando campo de Protocolo. Elemento: ${protocolo}, instância: ${Object.prototype.toString.call(protocolo).slice(8, -1) ?? "null"}`);
    }
    if (tabSVi &&
        ...
        const inpsWeight = tabMedAnt.querySelectorAll(".inpWeight") ||
            tabMedAnt.querySelectorAll(".tabCelRowMedAnt2");
        const inpsHeight = tabMedAnt.querySelectorAll(".inpHeight") ||
            tabMedAnt.querySelectorAll(".tabInpRowMedAnt3");
        const inpsSumDCuts = tabDC.querySelectorAll(".inpSumDCut") ||
                    ...
                    if (targInpWeight instanceof HTMLInputElement) {
                        ...
                        targInpWeight.addEventListener("input", () => {
                            ...
                            if (typeof returnedWeight === "number") {
                                ...
                                person.weight = parseInt(returnedWeight.replaceAll(/[^0-9.,+-]/g, "") || "0");
                            }
      ...
      Número obtido de propriedades: ${Object.keys(person).length ?? 0}; Número aceito: 6`);
        ...
        if (person instanceof Man ||
            ...
            TMBBtns.forEach((tmbbtn) => {
                tmbbtn.addEventListener("click", () => {
                    if (IMCMLGArray.length === 3) {
                        TMBArray = person.calcTMB(person, IMCMLGArray[1], factorAtleta, IMCMLGArray[2]) ?? ["", 0];
                    }
                    else {
            ...
            GETBtns.forEach((getbtn) => {
                getbtn.addEventListener("click", () => {
                    if (TMBArray.length === 2 && factorAtLvl) {
                        GET =
                            person.calcGET(TMBArray[1], factorAtLvl) ?? 0;
                    }
                    else {
                        console.error(`Valor de TMB obtido: ${TMBArray[1]};
            ...
            console.error(`Erro validando person. Elemento: ${person}; instância: ${Object.prototype.toString.call(person).slice(8, -1) ?? "null"}`);
        }
    }
    else {
      ...
      Tabela de Sinais Vitais: elemento ${JSON.stringify(tabSVi) ?? "null"}, instância ${Object.prototype.toString.call(tabSVi).slice(8, -1) ?? "null"}
        Tabela de Medidas Antropométricas: elemento ${JSON.stringify(tabMedAnt) ?? "null"}, instância: ${Object.prototype.toString
            .call(tabMedAnt)
            .slice(8, -1)}; 
    ...
    console.error(`Erro validando Tabela de Dobras Cutâneas: elemento ${tabDC}, instância ${Object.prototype.toString.call(tabDC).slice(8, -1) ?? "null"}`);
}
