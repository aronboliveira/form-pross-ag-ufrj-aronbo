...
export function numberLimit(inputElement: HTMLInputElement) {
  ...
  if ((isAtivFis || isAlimRot || isLocNum || isDDD || isFreq) && !isFloat) {
    if (numberValue.match(/[-+=.,;~\\\/|"!@#$%&*¬°ªº§¹²³£¢\(\)\{\}\[\]]/)) {
      const wrongMatch: string[] | null = numberValue.match(
        /[-+=.,;~\\\/|"!@#$%&*¬°ªº§¹²³£¢\(\)\{\}\[\]]/g
      );
...
export function autoCapitalizeInputs(textElement: textEl) {
  ...
  if (isAutocorrectOn && text) {
    ...
    let letterNotMatchesAfterD: string[] | null = text.match(
      ...
      if (
        ...
        function correctCursorNextWords(isUndoUppercase: boolean) {
          ...
          if (selectionPosition === 0) {
            wrongStartCorrection(wrongStartMatch);
            textElement.addEventListener("keyup", (fixmove: Event) => {
              if (
        ...
        function moveCursorToTheEnd(textElement: Element) {
...
export function autoCapitalizeCite(editableCite: HTMLElement) {
  ...
  if (isAutocorrectOn && citeText) {
    ...
    function capitalizeNextWords() {
      ...
      if (letterMatchesIniNotD && !letterMatchesIniD) {
        ...
        // console.log("foi chamado pelo capitalizeNext");
        correctCursorNextWords(isUndoUppercase);
        wrongStartCorrection(wrongStartMatch as string);
        arrayCheckLowerCases = Array.from(letterMatchesIniNotD);
        for (i = 0; i < arrayCheckLowerCases.length; i++) {
          filteredArray = letterMatchesIniNotD.filter((i) =>
            lowercasesRegexObj.test(i)
    ...
    function createSpanAlert(isSpanActive: boolean) {
    ...
    function moveCursorToTheEnd(editableCite: HTMLElement) {
      // console.log("chegou no moveCursor");
      if (window.getSelection && !isCursorAutoMoved) {
        let range = document.createRange();
        range.selectNodeContents(editableCite);
...
export function addEmailExtension(container: HTMLInputElement) {
  if (container instanceof HTMLInputElement || HTMLTextAreaElement) {
    ...
    ErrorHandler.inputNotFound(
      container ?? null,
      `${(container as HTMLElement)?.id ?? "UNDEFINED ID EMAIL CONTAINER"}`,
      slicedError ?? "NULL"
    );
  }
}
...
export function addDblQuotes(container: HTMLTextAreaElement) {
  if (container instanceof HTMLInputElement || HTMLTextAreaElement) {
    if (container.value === "") {
      container.value = '""';
    ...
    ErrorHandler.inputNotFound(
      container ?? null,
      `${(container as HTMLElement)?.id ?? "UNDEFINED ID QUOTED CONTAINER"}`,
      slicedError ?? "NULL"
    );
  }
}
