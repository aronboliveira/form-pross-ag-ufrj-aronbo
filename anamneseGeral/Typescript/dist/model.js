...
export function addEmailExtension(container) {
    if (container instanceof HTMLInputElement || HTMLTextAreaElement) {
        ...
        const splitError = error.stack?.split("\n");
        const slicedError = splitError[1].trim().slice(-7, -1);
        ErrorHandler.inputNotFound(container ?? null, `${container?.id ?? "UNDEFINED ID EMAIL CONTAINER"}`, slicedError ?? "NULL");
    }
}
...
export function addDblQuotes(container) {
    if (container instanceof HTMLInputElement || HTMLTextAreaElement) {
        ...
        const splitError = error.stack?.split("\n");
        const slicedError = splitError[1].trim().slice(-7, -1);
        ErrorHandler.inputNotFound(container ?? null, `${container?.id ?? "UNDEFINED ID QUOTED CONTAINER"}`, slicedError ?? "NULL");
    }
}
