...
export function getJSONDesc(inputs) {
    ...
    for (let l = 0; l < titleElements.length; l++) {
        ...
        //criação do storager
        const nJSONTitleStorager = new Classes.JSONTitleStorager(closestValidElements[l]);
        //criação da store
    ...
    if (JSONInpsStoreDescriptors.length === JSONInpsStore.length &&
        ...
        if (filter1JSONInpsStore.length === JSONInpsStore.length &&
            ...
            const filter2JSONInpsStore = JSONInpsStore.filter((JSONEl) => JSONEl instanceof Classes.JSONStorager);
            const filter2JSONTitlesStore = JSONTitlesStore.filter((JSONEl) => JSONEl instanceof Classes.JSONTitleStorager);
