...
export function getJSONDesc(inputs) {
    ...
    for (let j = 0; j < inputs.length; j++) {
        ...
        if (inpIds[j]?.match(/null/g) ||
            inpIds[j] === undefined ||
            inpIds[j] === null) {
            console.warn(`Id null detectada. Título relativo: ${closestValidElements[j]?.id ?? "null"}`);
        }
        //criação do storager
    ...
    for (let l = 0; l < titleElements.length; l++) {
        ...
        if (closestValidElements[l]?.match(/[Nn][Uu][Ll][Ll]/g) ||
            ...
            Título: ${closestValidElements[l]?.textContent ||
...
export function createJSONAnchor(JSONBtn, formInpsDescriptor, formTitlesDescriptor) {
    const formattedFormDescriptor = formatJSONFile(formInpsDescriptor, formTitlesDescriptor);
...
export function regenerateJSONBtn(JSONLink, formInpsDescriptor, formTitlesDescriptor) {
    ...
    setTimeout(() => {
        newJSONBtn.addEventListener("click", () => createJSONAnchor(newJSONBtn, formInpsDescriptor, formTitlesDescriptor));
    }, 1000);
    // return newJSONBtn;
}
export function opRadioHandler(keydown) {
    ...
    for (let i = 0; i < radioPairs.length; i += 2 //pulando de par em par
        ...
        if (radioYes instanceof HTMLInputElement &&
            ...
            ErrorHandler.multipleElementsNotFound(slicedError ?? "NULL", "validando radioYes ou radiosNo ou keydown event target", radioYes ?? null, radioNo ?? null);
        }
    }
}
...
export function addAntMedHandler(click) {
  ...
  `;
        ...
        for (let iB = 0; iB < dateBtns.length; iB++) {
            ...
            dateBtns[iB].addEventListener("click", (activation) => useCurrentDate(activation, dateBtns[iB]));
        }
    }
        ...
        const splitError = error.stack?.split("\n");
        const slicedError = splitError[1].trim().slice(-7, -1);
        ErrorHandler.elementNotFound(click.target ?? null, `${click.target.id ?? "UNDEFINED BUTTON ID"}`, slicedError ?? "NULL");
    }
}
export function useCurrentDate(activation, dateBtn) {
    ...
    if (activation.target === dateBtn &&
        ...
        const splitError = error.stack?.split("\n");
        const slicedError = splitError[1].trim().slice(-7, -1);
        ErrorHandler.inputNotFound(targInputDate ?? null, "targInputDate", slicedError ?? "NULL");
    }
}
        ...
        toFileInpBtns.forEach((toFileInpBtn) => {
            if (toFileInpBtn.textContent === "Retornar à Assinatura Escrita") {
                ...
                if (inpAst &&
                    ...
                    const splitError = error.stack?.split("\n");
                    const slicedError = splitError[1].trim().slice(-7, -1);
                    ErrorHandler.elementNotFound(inpAst ?? null, "inpAst", slicedError ?? "NULL");
                }
            }
        });
    }
